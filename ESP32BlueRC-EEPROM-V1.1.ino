/*******************************************************************
   ESP32 BlueRC Arduino Sketch V1.1
...
const uint16_t kRecvPin = 27; // receive pin to D6 (IO27)
const uint16_t kIrLed = 14;  // ESP32 GPIO pin to use. On BlueRC shield: D7 (IO14).
...
// DHT11 is connected to port IO17 (D4)
// blue led to port IO18 (D13)
#define LED1_blue 18
#define LED2_red 19
#define R 13
#define G 23
...
void updateDHT(){
  ...
  // send temprature and humiditz value to app via bluetooth
  String message="D,T: "+String(t,1)+"°C  H: "+String(h,1)+"%";
  SerialBT.println(message);
}

void setup()
{
  // setup pins and turn off all leds
...
void loop()
{
  ...
  if (SerialBT.available() > 0) // Incoming bluetooth message
  {
    ...
    {
      digitalWrite(LED1_blue, HIGH); // if learning, turn on LED1
      SerialBT.read();
      delay(10);
      ...
      if ((button > 47) && (button < 123)) // check received bluetooth command
      {
        ...
        delay(10);
        timeout=10000;
        while ((!irrecv.decode(&results))&&(timeout>0)) // waiting for ir signal
        {
          timeout--;
          delay(1);
    ...
    else // if only one char is received, send ir code
    {
      button = SerialBT.read();
      delay(10);
      if (button=='z') // request for updating dht data
      {
        updateDHT();
      }
      else
